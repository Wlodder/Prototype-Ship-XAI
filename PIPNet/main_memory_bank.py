...
def run_pipnet(args=None):
    ...
    for epoch in range(1, args.epochs + 1):                      
        ...
        if (epoch==args.epochs or epoch%30==0) and args.epochs>1:
            ...
            with torch.no_grad():
                ...
                print("Classifier weights: ", net.module._classification.weight[net.module._classification.weight.nonzero(as_tuple=True)], (net.module._classification.weight[net.module._classification.weight.nonzero(as_tuple=True)]).shape)
                if args.bias:
    ...
    if topks:
        ...
        print("Weights of prototypes", set_to_zero, "are set to zero because it is never detected with similarity>0.1 in the training set")
    ...
    print("classifier weights: ", net.module._classification.weight)
    print("Classifier weights nonzero: ", net.module._classification.weight[net.module._classification.weight.nonzero(as_tuple=True)], (net.module._classification.weight[net.module._classification.weight.nonzero(as_tuple=True)]).shape)
    print("Classifier bias: ", net.module._classification.bias)
    ...
    for c in range(net.module._classification.weight.shape[0]):
        ...
        if args.validation_size == 0.:
            print("Class", c, "(", list(testloader.dataset.class_to_idx.keys())[list(testloader.dataset.class_to_idx.values()).index(c)],"):","has", len(relevant_ps),"relevant prototypes: ", relevant_ps)
