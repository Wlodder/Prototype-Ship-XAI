...
def expand_pipnet_with_pure_centroids_trajectory(original_model, results, device='cuda', scalings=[0.5, 1.0, 2.0]):
    """
...
def find_on_manifold_prototypes(model, dataloader, centroids, device='cuda'):
    """Find real data points closest to computed centroids"""
    closest_points = [None] * len(centroids)
    min_distances = [float('inf')] * len(centroids)
    
    # Convert centroids to tensor
    centroid_tensors = [torch.tensor(c, device=device) for c in centroids]
    
    # Scan dataset to find closest real points
    with torch.no_grad():
        for batch in tqdm(dataloader):
            inputs = batch[0].to(device)
            
            # Extract features
            _, _, _, features = model(inputs, features_save=True)
            
            # For each sample, compute distance to centroids
            b, c, h, w = features.shape
            features_flat = features.reshape(b, c, h*w).permute(0, 2, 1)  # [B, HW, C]
            
            for i in range(b):
                for pos in range(features_flat.shape[1]):
                    feature_vector = features_flat[i, pos]
                    
                    # Check distance to each centroid
                    for j, centroid in enumerate(centroid_tensors):
                        # Compute distance
                        distance = torch.sum((feature_vector - centroid.flatten())**2).item()
                        
                        if distance < min_distances[j]:
                            min_distances[j] = distance
                            # Store the point and its metadata
                            closest_points[j] = {
                                'sample': inputs[i].cpu(),
                                'position': (pos // w, pos % w),
                                'feature': feature_vector.cpu()
                            }
