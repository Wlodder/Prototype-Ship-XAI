...
CHECKPOINT=$LOGDIR/checkpoints/net_pretrained

mkdir -p $LOGDIR
mkdir -p $IMGDIR

python $PIPNET_RUNDIR/main_gumbel.py --dataset "Janes_Military_Marvel" --net $NET --batch_size $BATCH_SIZE\
 ...
 --buffer_type buffer:35 --k 45 --patch_size 32 --state_dict_dir_net $CHECKPOINT\
  --debug --feature_folder $LOGDIR/visualised_prototypes_next
