...
SUBJECT_DIR=${NET}_${EPOCHS_PRETRAIN}_${EPOCHS_TRAIN}_${NUM_FEATURES}_${BATCH_SIZE}_${BATCH_SIZE_PRETRAIN}_${IMAGE_SIZE}/${JANES_EXPERIMENT_DIRECTORY}/${K}_${BUFFER_SIZE}_main

# Create log dir and image dir
LOGDIR=$PIPNET_PROTO_DIR/$SUBJECT_DIR/log
IMGDIR=$PIPNET_PROTO_DIR/$SUBJECT_DIR/img_dir
CHECKPOINT=$LOGDIR/checkpoints/net_trained_8

mkdir -p $LOGDIR
mkdir -p $IMGDIR

python $PIPNET_RUNDIR/get_image_clusters.py --dataset "Janes_Military_Marvel" --net $NET --batch_size $BATCH_SIZE\
 ...
 --buffer_type buffer:$BUFFER_SIZE --k 25 --patch_size 32 --state_dict_dir_net $CHECKPOINT
